...
import { Plus, Upload, X, Sparkles, Search, Loader2, ChevronDown, ChevronUp, Cpu, Monitor, HelpCircle, FileText, AlertTriangle } from 'lucide-react';
...
const getSteamGameInfoFromInput = async (input) => {
  const query = cleanText(input);
  const appId = getSteamAppIdFromUrl(query) || (/^\d+$/.test(query) ? query : '');
  const steamInfo = appId
    ? await getSteamInfoByAppId(appId, '')
  ...
  if (!steamInfo) {
    throw new Error('Không tìm thấy game trên Steam. Với game 18+ khó tìm bằng tên, hãy dán link Steam hoặc appId.');
  }
...
function Admin() {
  ...
  const handleRawgAiGenerateAll = async (gamesToProcess = null) => {
    ...
    if (!key) {
      alert('Vui lòng cấu hình Gemini API Key ở trên để sử dụng tính năng viết bài AI!');
      return;
    }
    const targetGames = gamesToProcess || rawgResults;
    if (targetGames.length === 0) return;
    
    // Tạo bài viết lần lượt cho từng game để tránh quá tải
  ...
  const handleApplyRawgToForm = (game, aiData) => {
    ...
    window.scrollTo({ top: document.getElementById('admin-game-form')?.offsetTop - 80, behavior: 'smooth' });
        ...
        <div className="card" id="admin-game-form">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', marginBottom: '1.5rem', padding: '1rem', backgroundColor: 'rgba(255, 255, 255, 0.02)', border: '1px solid var(--color-border)', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <input type="text" className="input-field" placeholder="Nhập tên game trên Steam, link Steam hoặc appId..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ flex: 1 }} />
            ...
            <span style={{ color: 'var(--color-text-muted)', fontSize: '0.75rem' }}>Ưu tiên tuyệt đối nguồn Steam: ảnh, screenshot, mô tả và cấu hình lấy từ Steam; Gemini chỉ dịch/tóm tắt sang tiếng Việt.</span>
          </div>
                ...
                {rawgResults.map((game) => (
                  <div key={game.id} className="gs-game-card">
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', padding: '0 1rem 1rem', flexWrap: 'wrap' }}>
                      ...
                      {rawgAiContent[game.id] && (
                        <button
                          onClick={() => handleApplyRawgToForm(game, rawgAiContent[game.id])}
                          style={{
                            flex: 1,
